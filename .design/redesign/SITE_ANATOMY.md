...
whole site is static files". Carries a JSON-LD `@graph` of `SoftwareApplication`
priced at 0 and a `BreadcrumbList` matching the trail above the `<h1>` (three
items on a tool page, four on a preset).
...
| Element | Source |
| --- | --- |
| `<title>` | `t.toolTitle(name)`, or `seoTitle`, or `t.categoryTitle(modifier)` |
...
| JSON-LD | `WebSite` with `SearchAction` on the homepage, `CollectionPage` with `BreadcrumbList` on category and collection pages, `SoftwareApplication` + `BreadcrumbList` on tool and preset pages. Every page that draws a breadcrumb declares one: 128 of them. |
