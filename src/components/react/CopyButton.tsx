...
export default function CopyButton({ text, label = 'Copy', copiedLabel = 'Copied', announce = 'Copied to clipboard' }: {
  ...
  return (
    <button
      ...
      onClick={copy}
      disabled={!text}
      className="rounded border border-border px-2 py-1 text-2xs font-medium text-muted pointer-coarse:min-h-11 pointer-coarse:px-3.5 pointer-coarse:text-sm transition-colors duration-[120ms] hover:border-border-strong hover:text-text disabled:opacity-40 disabled:hover:border-border disabled:hover:text-muted"
    >
