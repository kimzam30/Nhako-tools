...
export const run: FileRun = async (files, opts, ctx) => {
  ...
  if (converted.length === 1) {
    return { blob: first.blob, filename: first.name, summary: say(`Converted to ${EXTENSION[mime].toUpperCase()}`, `Ditukar kepada ${EXTENSION[mime].toUpperCase()}`) };
  }
  ...
  return {
    ...
    summary: say(
      `${converted.length} images converted to ${EXTENSION[mime].toUpperCase()}`,
      `${converted.length} imej ditukar kepada ${EXTENSION[mime].toUpperCase()}`,
    ),
  };
};
